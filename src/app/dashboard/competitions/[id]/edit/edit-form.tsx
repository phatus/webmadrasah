...
import { updateCompetition } from "@/actions/competition"
...
function SubmitButton() {
    ...
    return (
        ...
        >
            {pending ? 'Menyimpan...' : 'Update Lomba'}
        </button>
    )
}
...
export default function EditCompetitionForm({ competition }: EditCompetitionFormProps) {
    ...
    const [imageUrl, setImageUrl] = useState(competition.image || '')
                        ...
                        <div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
                            <div className="w-full xl:w-1/2">
                                ...
                                <input
                                    ...
                                    name="startDate"
                                    required
                                    defaultValue={new Date(competition.startDate).toISOString().split('T')[0]}
                            ...
                            <div className="w-full xl:w-1/2">
                                ...
                                <input
                                    ...
                                    name="endDate"
                                    required
                                    defaultValue={new Date(competition.endDate).toISOString().split('T')[0]}
                                    className="w-full rounded border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-emerald-600 active:border-emerald-600"
                                />
                            </div>
                        </div>
                            ...
                            <label className="flex items-center cursor-pointer select-none group">
                                <div className="relative">
                                    <input
                                        type="checkbox"
                                        name="isActive"
                                        defaultChecked={competition.isActive}
                                        className="sr-only"
                                    />
