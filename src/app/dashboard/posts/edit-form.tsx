    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (err?.message !== 'NEXT_REDIRECT') {
                console.error(err)
                setErrorMessage('Terjadi kesalahan saat menguji simpan berita.')
                setIsSubmitting(false)
            }
        }
    }
                        ...
                        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
                            <label className="flex items-center cursor-pointer select-none">
                                ...
                                <div className="ml-4 font-semibold text-sm text-slate-800">
                                    {isPublished ? (
                                        <span className="text-emerald-700">Publikasikan Sekarang <span className="text-xs text-slate-500 font-normal">(Berita tampil publik di website)</span></span>
                                    ) : (
                        ...
                        >
                            {isSubmitting ? (
                                <>
                                    ...
                                    <span>Memperbarui & Mempublikasikan Berita...</span>
                                </>
                            ) : (
                                <span>{isPublished ? 'Update & Publikasikan Berita' : 'Update Draf Berita'}</span>
                            )}
                        </button>
                    </div>
                </form>
